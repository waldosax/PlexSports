...
pfr_current_season = str(__now.year - 1 if __now.month < 8 else __now.year)
pfr_last_season = str(__now.year - 2 if __now.month < 8 else __now.year - 1)

pfr_league_defs = dict()
pfr_year_hrefs = dict()
pfr_year_superbowls = dict()
...
def Scrape():
	...
	years = pfr_year_hrefs[league].keys()
	years.sort(reverse=True)
	pfr_current_season = years[0]
	pfr_last_season = years[1]
...
def __process_years_index_page():
	...
	soup = BeautifulSoup(GetResultFromNetwork(url, cacheExtension=EXTENSION_HTML), "html5lib")
		...
		if not ld.attrs.get("data-stat") in ["year_id", "summary"]:
			...
			for ldNode in ldSoup.select("b"):
				if not ldNode.next == "League" and ldNode.next and ldNode.next.next:
					...
					while value[:1] in [" ", "-"]:
						value = value[1:]
					pfr_league_defs[key] = value
	#pprint(pfr_league_defs)
	...
	for yearRow in yearRows:
		...
		for yearLeague in yearLeagues:
			...
			pfr_year_hrefs.setdefault(yearLeagueText, dict())
			pfr_year_hrefs[yearLeagueText][yearText] = yearLeagueHref
		...
		if yearSuperbowls:
			...
			pfr_year_superbowls[yearText] = yearSuperbowlText.rstrip(": ")

	#pprint(pfr_year_hrefs)
	#pprint(pfr_year_superbowls)
	pass
...
def __process_nfl_season_page(year):
	...
	url = PFR_BASE_URL + pfr_year_hrefs[LEAGUE_NFL][year]
	soup = BeautifulSoup(GetResultFromNetwork(url, cacheExtension=EXTENSION_HTML), "html5lib")
	...
	for weekSummary in weekSummaries:
		...
		if weekSummaryText[:4] == "Week":
			...
			pfr_schedule_weeks[LEAGUE_NFL][year]["subseasons"][1][4]["alias"] = pfr_year_superbowls.get(year) or "Superbowl"
...
def __process_nfl_game_page(event):
	...
	for teamName in teams.keys():
		...
		elif isActive == None and year in [pfr_current_season, pfr_last_season]: isActive = True
